...
public class RCC_UIDashboardButton : MonoBehaviour {
	...
	private Death_Race_CarControllerV3[] carControllers;
	...
	public void OnClicked () {
		...
		case ButtonType.LeftIndicator:
			...
			for(int i = 0; i < carControllers.Length; i++){
				...
				if(carControllers[i].canControl && !carControllers[i].AIController){
					if(carControllers[i].indicatorsOn != Death_Race_CarControllerV3.IndicatorsOn.Left)
						carControllers[i].indicatorsOn = Death_Race_CarControllerV3.IndicatorsOn.Left;
					else
						carControllers[i].indicatorsOn = Death_Race_CarControllerV3.IndicatorsOn.Off;
				}
		...
		case ButtonType.RightIndicator:
			...
			for(int i = 0; i < carControllers.Length; i++){
				...
				if(carControllers[i].canControl && !carControllers[i].AIController){
					if(carControllers[i].indicatorsOn != Death_Race_CarControllerV3.IndicatorsOn.Right)
						carControllers[i].indicatorsOn = Death_Race_CarControllerV3.IndicatorsOn.Right;
					else
						carControllers[i].indicatorsOn = Death_Race_CarControllerV3.IndicatorsOn.Off;
				}
		...
		case ButtonType.HazardLights:
			...
			for(int i = 0; i < carControllers.Length; i++){
				...
				if(carControllers[i].canControl && !carControllers[i].AIController){
					if(carControllers[i].indicatorsOn != Death_Race_CarControllerV3.IndicatorsOn.All)
						carControllers[i].indicatorsOn = Death_Race_CarControllerV3.IndicatorsOn.All;
					else
						carControllers[i].indicatorsOn = Death_Race_CarControllerV3.IndicatorsOn.Off;
				}
...
//					carControllers[i].StartCoroutine("ChangingGear", 0);
	...
	void OnDisable(){
		...
		if(_buttonType == ButtonType.Gear){
			...
			foreach(Death_Race_CarControllerV3 rcc in carControllers){
