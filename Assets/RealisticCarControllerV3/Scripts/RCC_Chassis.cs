...
public class RCC_Chassis : MonoBehaviour {
	...
	void Start () {
		...
		mainRigid = GetComponentInParent<Death_Race_CarControllerV3> ().GetComponent<Rigidbody> ();

		chassisVerticalLean = GetComponentInParent<Death_Race_CarControllerV3> ().chassisVerticalLean;
		chassisHorizontalLean = GetComponentInParent<Death_Race_CarControllerV3> ().chassisHorizontalLean;
	...
	void ChassisJoint(){
		...
		colliders.transform.SetParent(GetComponentInParent<Death_Race_CarControllerV3> ().transform, false);

		GameObject chassisJoint;

		Transform[] childTransforms = GetComponentInParent<Death_Race_CarControllerV3> ().chassis.GetComponentsInChildren<Transform>();
